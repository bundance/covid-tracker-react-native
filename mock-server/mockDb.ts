...
import { Patient, Assessment, Contributions, CovidTest, Consent, StudyConsent } from './types';

const dbPath = './mock-server/db';

export default () => {
  const { bootstrap, get, save } = helpers(dbPath);
  const { patients, assessments, covidTests, contributions, consents, studyConsents } = config;
